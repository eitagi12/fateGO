...
export enum ErrorPerSoSimMessage {
  ...
  ERROR_OMNI_MESSAGE = 'ไม่สามารถให้บริการได้ กรุณาติดต่อพนักงานเพื่อดำเนินการ ขออภัยในความไม่สะดวก',
...
export class OmniNewRegisterPersoSimPageComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    console.log('==================> 111111111111111', this.data.cusMobileNo);
  }
  ...
  startPersoSim(transaction: Transaction | Partial<Transaction>): void {
    ...
    this.persoSimSubscription = this.onPersoSim(this.data.cusMobileNo).subscribe((value) => {
      console.log('==================> 222222222222', this.data.cusMobileNo);
      this.isNext = false;
      this.persoSim = value;
      this.title = value.eventName;
      if (value.queryPersoSim) {
        this.data.cusMobileNo = Object.assign(this.data.cusMobileNo, {
      ...
      if (value.error) {
        ...
        if (value.error.errorCase === ErrorPerSoSim.ERROR_SIM) {
          ...
          if (this.checkErrSim < 3) {
            ...
            this.alertService.error(this.translation.instant(ErrorPerSoSimMessage.ERROR_OMNI_MESSAGE));
            this.errorMessage = ErrorPerSoSimMessage.ERROR_OMNI_MESSAGE;
          }
        } else
  ...
  onPersoSim(mobileNo: string): Observable<PersoSim> {
    this.mobileNo = this.data.cusMobileNo;
                          ...
                          .then((persoSimCommand: ControlSimResult) => {
                            ...
                            if (persoMassege[0] === PersoSimCardStatus.EVENT_PERSO_SIM_SUCCESS) {
                              this.checkCreatePersoSim(this.referenceNumber).then((crestePersoSim) => {
                                ...
                                this.checkOmniStatusCompleted(this.referenceNumber).then((resOrder) => {
                                  if (resOrder) {
          ...
          this.controlSim(ControlSimCard.EVENT_KEEP_SIM).then(() => {
            this.controlSim(ControlLED.EVENT_LED_OFF).then(() => {
              ...
              errNext = {
                progress: 0,
                eventName: '',
                error: { errorCase: ErrorPerSoSim.ERROR_OMNI, messages: messages ? messages : ErrorPerSoSimMessage.ERROR_OMNI_MESSAGE }
              };
              resolve(errNext);
            });
          });
  ...
  private checkOmniStatusCompleted(referenceNumber: string): Promise<any> {
    return new Promise((resolve) => {
      let nubCount = 0;
      const checkOmni = setInterval(() => {
        this.checkOmniStatus(referenceNumber).then((respone) => {
          if (respone.data.orderStatus === 'Completed') {
            clearInterval(checkOmni);
            resolve(respone);
          } else {
            nubCount++;
            if (nubCount > 20) {
  ...
  checkOmniStatus(refNo: string): Promise<any> {
    const checkOmniStatusAPI = '/api/customerportal/newRegister/checkOmniStatus?refNo=' + refNo;
    return this.http.get(checkOmniStatusAPI).toPromise();
  }
