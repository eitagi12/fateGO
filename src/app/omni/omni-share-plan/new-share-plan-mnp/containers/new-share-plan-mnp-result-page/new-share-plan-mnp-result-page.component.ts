...
export class NewSharePlanMnpResultPageComponent implements OnInit {
    ...
    this.createNewRegisterService.createNewRegister(this.transaction)
      .then((resp: any) => {
        ...
        if (this.transaction.data.order.orderNo) {
          this.isSuccess = true;
          this.checkOmniStatusByOmniNo(this.transaction.data.order.orderNo).then((res: any) => {
            ...
            this.alertService.notify({
              type: 'error',
              html: `ขออภัย ทำรายการไม่สำเร็จ ให้ทำรายการใหม่อีกครั้ง <br> (OMNI : ${this.transaction.data.order.orderNo})`,
  ...
  checkOmniStatusByOmniNo(orderNo: string): Promise<any> {
      ...
      map((res: any) => {
        if (res.data.orderNo === orderNo) {
          if (res.data.orderStatus === 'Completed') {
            ...
            throw new Error('Omni status not completed');
          }
        } else {
          throw new Error('Omni status not completed');
        }
