...
import { ROUTE_DEVICE_ORDER_ASP_SHARE_PLAN_NEW_REGISTER_MNP_VALIDATE_CUSTOMER_ID_CARD_PAGE, ROUTE_DEVICE_ORDER_ASP_SHARE_PLAN_NEW_REGISTER_MNP_VALIDATE_CUSTOMER_KEY_IN_PAGE, ROUTE_DEVICE_ORDER_ASP_SHARE_PLAN_NEW_REGISTER_MNP_PAYMENT_DETAIL_PAGE } from '../../constants/route-path.constant';
...
export class NewRegisterMnpValidateCustomerPageComponent implements OnInit, OnDestroy {
  ...
  onReadCard(): void {
    this.router.navigate([ROUTE_DEVICE_ORDER_ASP_SHARE_PLAN_NEW_REGISTER_MNP_VALIDATE_CUSTOMER_ID_CARD_PAGE]);
  }
  ...
  onNext(): void {
    ...
    this.validateCustomer().catch((error: any) => {
      if (error.error.developerMessage === 'EB0001 : Data Not Found.') {
        ...
        this.router.navigate([ROUTE_DEVICE_ORDER_ASP_SHARE_PLAN_NEW_REGISTER_MNP_VALIDATE_CUSTOMER_KEY_IN_PAGE], {
          queryParams: {
        ...
        return this.validateCustomerService.checkValidateCustomer(this.identity, cardType, transactionType)
          .then((customer: any) => {
            ...
            if (err.error.resultCode === 'MYCHN00150006') {
              ...
              this.alertService.error(JSON.stringify(err.error.errors[0]));
            }
          });
      });
  }
