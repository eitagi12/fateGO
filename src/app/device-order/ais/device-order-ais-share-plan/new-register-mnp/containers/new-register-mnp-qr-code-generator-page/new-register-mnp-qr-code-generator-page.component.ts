...
@Component({
  ...
  templateUrl: './new-register-mnp-qr-code-generator-page.component.html',
  styleUrls: ['./new-register-mnp-qr-code-generator-page.component.scss']
})
export class NewRegisterMnpQrCodeGeneratorPageComponent implements OnInit, OnDestroy {
  ...
  onNext(): void {
    const mpayStatus = this.transaction.data.mpayPayment.mpayStatus;
    if (this.priceOption.productStock.company === 'AWN') {
      this.router.navigate([ROUTE_DEVICE_ORDER_AIS_SHARE_PLAN_NEW_REGISTER_MNP_QR_CODE_QUEUE_PAGE]);
    } else {
