...
import { ROUTE_DEVICE_ORDER_AIS_BEST_BUY_CHECK_OUT_PAGE, ROUTE_DEVICE_ORDER_AIS_BEST_BUY_OMISE_GENERATOR_PAGE } from 'src/app/device-order/ais/device-order-ais-existing-best-buy/constants/route-path.constant';

@Component({
  selector: 'app-device-order-ais-existing-best-buy-omise-summary-page',
  templateUrl: './device-order-ais-existing-best-buy-omise-summary-page.component.html',
  styleUrls: ['./device-order-ais-existing-best-buy-omise-summary-page.component.scss']
})
export class DeviceOrderAisExistingBestBuyOmiseSummaryPageComponent implements OnInit, OnDestroy {
  ...
  onNext(): void {
    ...
    this.qrCodeOmisePageService.createOrder(params).then((res) => {
      ...
      this.router.navigate([ROUTE_DEVICE_ORDER_AIS_BEST_BUY_OMISE_GENERATOR_PAGE]);
