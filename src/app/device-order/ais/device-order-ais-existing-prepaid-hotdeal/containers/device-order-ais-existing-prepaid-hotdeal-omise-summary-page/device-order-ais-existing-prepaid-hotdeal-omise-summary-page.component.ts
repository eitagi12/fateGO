...
import { ROUTE_DEVICE_ORDER_AIS_PREPAID_HOTDEAL_AGGREGATE_PAGE, ROUTE_DEVICE_ORDER_AIS_PREPAID_HOTDEAL_OMISE_ERROR_PAGE, ROUTE_DEVICE_ORDER_AIS_PREPAID_HOTDEAL_OMISE_GENERATOR_PAGE } from '../../constants/route-path.constant';

@Component({
  selector: 'app-device-order-ais-existing-prepaid-hotdeal-omise-summary-page',
  templateUrl: './device-order-ais-existing-prepaid-hotdeal-omise-summary-page.component.html',
  styleUrls: ['./device-order-ais-existing-prepaid-hotdeal-omise-summary-page.component.scss']
})
export class DeviceOrderAisExistingPrepaidHotdealOmiseSummaryPageComponent implements OnInit, OnDestroy {
  ...
  onNext(): void {
    ...
    if (!this.transaction.data.omise.qrCodeStr) {
      this.qrCodeOmisePageService.createOrder(params).then((res) => {
        ...
        this.router.navigate([ROUTE_DEVICE_ORDER_AIS_PREPAID_HOTDEAL_OMISE_GENERATOR_PAGE]);
