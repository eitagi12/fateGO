...
import { ROUTE_DEVICE_ORDER_AIS_EXISTING_OMISE_RESULT_PAGE } from '../../constants/route-path.constant';
...
@Component({
  selector: 'app-device-order-ais-existing-omise-queue-page',
  templateUrl: './device-order-ais-existing-omise-queue-page.component.html',
  styleUrls: ['./device-order-ais-existing-omise-queue-page.component.scss']
})
export class DeviceOrderAisExistingOmiseQueuePageComponent implements OnInit, OnDestroy {
  ...
  onNext(): void {
    ...
    this.queuePageService.getQueueQmatic(this.queueFrom.value.mobileNo)
      ...
      .then(() => {
        this.router.navigate([ROUTE_DEVICE_ORDER_AIS_EXISTING_OMISE_RESULT_PAGE]);
      })
