...
import { ROUTE_DEVICE_ORDER_AIS_EXISTING_OMISE_GENERATOR_PAGE, ROUTE_DEVICE_ORDER_AIS_EXISTING_AGGREGATE_PAGE } from 'src/app/device-order/ais/device-order-ais-existing/constants/route-path.constant';

@Component({
  selector: 'app-device-order-ais-existing-omise-summary-page',
  templateUrl: './device-order-ais-existing-omise-summary-page.component.html',
  styleUrls: ['./device-order-ais-existing-omise-summary-page.component.scss']
})
export class DeviceOrderAisExistingOmiseSummaryPageComponent implements OnInit, OnDestroy {
  ...
  onNext(): void {
    ...
    const params: any = {
      ...
      this.qrCodeOmisePageService.createOrder(params).then((res) => {
        ...
        this.router.navigate([ROUTE_DEVICE_ORDER_AIS_EXISTING_OMISE_GENERATOR_PAGE]);
