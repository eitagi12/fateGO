...
import { ReadCardProfile, User, ValidateCustomerIdCardComponent, Utils, HomeService, AlertService, PageLoadingService, TokenService } from 'mychannel-shared-libs';
import { PriceOption } from 'src/app/shared/models/price-option.model';
import { Router } from '@angular/router';
import { CustomerInfoService } from 'src/app/device-order/services/customer-info.service';
import { TransactionService } from 'src/app/shared/services/transaction.service';
import { PriceOptionService } from 'src/app/shared/services/price-option.service';
import { ROUTE_DEVICE_ORDER_AIS_BEST_BUY_SHOP_VALIDATE_CUSTOMER_REPI_PAGE, ROUTE_DEVICE_ORDER_AIS_BEST_BUY_SHOP_PAYMENT_DETAIL_PAGE, ROUTE_DEVICE_ORDER_AIS_BEST_BUY_SHOP_CUSTOMER_PROFILE_PAGE } from '../../constants/route-path.constant';

@Component({
  selector: 'app-device-order-ais-existing-best-buy-shop-validate-customer-id-card-repi-page',
  templateUrl: './device-order-ais-existing-best-buy-shop-validate-customer-id-card-repi-page.component.html',
  styleUrls: ['./device-order-ais-existing-best-buy-shop-validate-customer-id-card-repi-page.component.scss']
})
export class DeviceOrderAisExistingBestBuyShopValidateCustomerIdCardRepiPageComponent implements OnInit, OnDestroy {
              ...
              if (respPrepaidIdent) {
                ...
                if (!this.utils.isIdCardExpiredDate(expireDate)) {
                  ...
                  if (simCard.chargeType === 'Pre-paid') {
                    this.pageLoadingService.closeLoading();
                    this.router.navigate([ROUTE_DEVICE_ORDER_AIS_BEST_BUY_SHOP_CUSTOMER_PROFILE_PAGE]);
                  } else {
                    this.pageLoadingService.closeLoading();
