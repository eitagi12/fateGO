...
import { ROUTE_DEVICE_ORDER_AIS_BEST_BUY_SHOP_OMISE_RESULT_PAGE } from 'src/app/device-order/ais/device-order-ais-existing-best-buy-shop/constants/route-path.constant';

@Component({
  selector: 'app-device-order-ais-existing-best-buy-shop-omise-queue-page',
  templateUrl: './device-order-ais-existing-best-buy-shop-omise-queue-page.component.html',
  styleUrls: ['./device-order-ais-existing-best-buy-shop-omise-queue-page.component.scss']
})
export class DeviceOrderAisExistingBestBuyShopOmiseQueuePageComponent implements OnInit, OnDestroy {
  ...
  onNext(): void {
    ...
    this.queuePageService.getQueueQmatic(this.queueFrom.value.mobileNo)
      ...
      .then(() => {
        this.router.navigate([ROUTE_DEVICE_ORDER_AIS_BEST_BUY_SHOP_OMISE_RESULT_PAGE]);
      })
