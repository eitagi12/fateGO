...
import { ROUTE_DEVICE_ORDER_AIS_BEST_BUY_SHOP_CAPTURE_REPI_PAGE, ROUTE_DEVICE_ORDER_AIS_BEST_BUY_SHOP_PAYMENT_DETAIL_PAGE, ROUTE_DEVICE_ORDER_AIS_BEST_BUY_SHOP_CUSTOMER_PROFILE_PAGE } from '../../constants/route-path.constant';

@Component({
  selector: 'app-device-order-ais-existing-best-buy-shop-otp-page',
  templateUrl: './device-order-ais-existing-best-buy-shop-otp-page.component.html',
  styleUrls: ['./device-order-ais-existing-best-buy-shop-otp-page.component.scss']
})
export class DeviceOrderAisExistingBestBuyShopOtpPageComponent implements OnInit {
  ...
  navigateNext(): void {
    if (this.transaction.data.action === TransactionAction.READ_CARD_REPI) {
      ...
      this.pageLoadingService.closeLoading();
      this.router.navigate([ROUTE_DEVICE_ORDER_AIS_BEST_BUY_SHOP_CAPTURE_REPI_PAGE]);
    }
  }
    ...
    this.customerInfoService.callUpdatePrepaidIdentify(this.transaction.data.customer, this.transaction.data.simCard.mobileNo)
      .then((response: any) => {
        ...
        if (response && response.data && response.data.success) {
          ...
          this.alertService.error('ระบบไม่สามารถแสดงตนได้');
        }
      }).catch((error) => {
        this.pageLoadingService.closeLoading();
        this.alertService.error('รหัส OTP ไม่ถูกต้อง กรุณาระบุใหม่อีกครั้ง');
