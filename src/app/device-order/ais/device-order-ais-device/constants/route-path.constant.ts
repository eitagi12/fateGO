export const ROUTE_DEVICE_AIS_DEVICE_PAYMENT_PAGE = '/device-order/ais/device/payment';
export const ROUTE_DEVICE_AIS_DEVICE_EDIT_BILLING_ADDRESS_PAGE = '/device-order/ais/device/ebilling-address';
export const ROUTE_DEVICE_AIS_DEVICE_SUMMARY_PAGE = '/device-order/ais/device/summary';
export const ROUTE_DEVICE_AIS_DEVICE_EDIT_SHIPPING_ADDRESS_PAGE = '/device-order/ais/device/eshipping-address';

export const ROUTE_DEVICE_AIS_DEVICE_QUEUE_PAGE = '/device-order/ais/device/queue';
export const ROUTE_DEVICE_AIS_DEVICE_RESULT_PAGE = '/device-order/ais/device/result';
export const ROUTE_DEVICE_AIS_DEVICE_AGGREGATE_PAGE = '/device-order/ais/device/aggregate';

export const ROUTE_DEVICE_AIS_DEVICE_QR_CODE_SUMMARY_PAGE = '/device-order/ais/device/qr-code-summary';
export const ROUTE_DEVICE_AIS_DEVICE_QR_CODE_GENERATOR_PAGE = '/device-order/ais/device/qr-code-generator';
export const ROUTE_DEVICE_AIS_DEVICE_QR_CODE_QUEUE_PAGE = '/device-order/ais/device/qr-code-queue';
export const ROUTE_DEVICE_AIS_DEVICE_QR_CODE_RESULT_PAGE = '/device-order/ais/device/qr-code-result';
export const ROUTE_DEVICE_AIS_DEVICE_OMISE_SUMMARY_PAGE = '/device-order/ais/device/omise-summary';
export const ROUTE_DEVICE_AIS_DEVICE_OMISE_GENERATOR_PAGE = '/device-order/ais/device/omise-generator';
export const ROUTE_DEVICE_AIS_DEVICE_OMISE_QUEUE_PAGE = '/device-order/ais/device/omise-queue';
