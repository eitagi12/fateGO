...
export class SignedApi {
    ...
    readonly ON_SCREEN_SIGNPAD = 'OnscreenSignpad';
}
...
export class AisNativeOrderService {
   ...
   private barcode: Subject<string> = new Subject<string>();
    ...
    readonly ERROR_BROWSER_NOT_SUPPORTED = 'เว็บเบราว์เซอร์นี้ไม่รองรับการเซ็นลายเซ็น';
    ...
    */
    ...
    printThermalTradeIn(data: string) {
    ...
    private barcodeCallback(barcode: any) {
    ...
    private cameraCallback(xml: string) {
        this._ngZone.run(() => {
            const parser: any = new DOMParser();
    ...
    private signatureCallback(signatureImage: string) {
        this._ngZone.run(() => {
            const parser: any = new DOMParser();
